...
struct Normalizer {
    ...
    Normalizer(std::vector<Triangle> &triangles) {
        ...
        normalizeMat = glm::translate(glm::scale(glm::mat4(1), glm::vec3(scale / diff[scaleAxis])), -minAxis);

        for (auto &t: triangles) {
            for (size_t i = 0; i < 3; i++) {
                t[i] = normalizeMat * glm::vec4(t[i], 1);
//                t[i] = scale * (t[i] - minAxis) / (diff[scaleAxis]);
        ...
        center = (maxAxis - minAxis) / 2.f / diff[scaleAxis];
    }
};